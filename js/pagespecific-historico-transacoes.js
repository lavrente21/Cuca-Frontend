// frontend/js/pageSpecific-historico-transacoes.js
...
import { API_BASE_URL } from './config.js'; // Agora usando a pasta js
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadTransactions(filter = 'all') {
        ...
        if (!userToken) {
            ...
            showLoading(false);
            setTimeout(() => { window.location.href = 'Login.html'; }, 2000); // Caminho relativo
            return;
        }
        ...
        try {
            ...
            const withdrawalsResponse = await fetch(`${API_BASE_URL}/api/withdrawals/history`, {
                method: 'GET',
                headers: {
            ...
            const depositsResponse = await fetch(`${API_BASE_URL}/api/deposits/history`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${userToken}`
            ...
            const investmentsResponse = await fetch(`${API_BASE_URL}/api/investments/history`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${userToken}`
